...
const Avatar: React.FC<AvatarProps> = forwardRef<HTMLDivElement, AvatarProps>(
  ({ size = "m", value, src, loading, empty, statusIndicator, style, className }, ref) => {
    ...
    const renderContent = () => {
      ...
      if (src) {
        return <SmartImage src={src} fill alt="Avatar" sizes={`${sizeMapping[size]}px`} className={styles.image} />;
      }
